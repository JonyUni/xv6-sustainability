...
main(int argc, char *argv[])
{
  ...
  if(argc > 1){
    ...
    if(read(gatefd, &gate, 1) != 1){
      printf("shortjob: start gate failed\n");
      exit(1);
    }
    close(gatefd);
  }
  ...
  start = uptime();
  printf("shortjob start pid=%d tick=%d\n", getpid(), start);
  burn(2400);
  end = uptime();

  if(getschedstats(&stats) < 0){
    printf("shortjob: getschedstats failed\n");
    exit(1);
  }

  printf("shortjob done pid=%d start=%d end=%d wall=%d run_ticks=%d picks=%d last_burst=%d\n",
