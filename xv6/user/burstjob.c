...
main(int argc, char *argv[])
{
  ...
  if(argc > 1){
    ...
    if(read(gatefd, &gate, 1) != 1){
      printf("burstjob: start gate failed\n");
      exit(1);
    }
    close(gatefd);
  }
  ...
  start = uptime();
  printf("burstjob start pid=%d tick=%d\n", getpid(), start);
  ...
  if(getschedstats(&stats) < 0){
    printf("burstjob: getschedstats failed\n");
    exit(1);
  }

  printf("burstjob done pid=%d start=%d end=%d wall=%d run_ticks=%d picks=%d last_burst=%d\n",
